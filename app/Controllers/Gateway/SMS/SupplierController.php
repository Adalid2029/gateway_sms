...
class SupplierController extends ResourceController
{
    ...
    public function pendingMessages(string $channel = 'SMS')
    {
        ...
        if (!$pendingMessage) {
            ...
            $this->logger->alert(
                "PROVIDER_ACTIVITY - ID: {$this->user->id} - CHANNEL: {$channel} - ACTION: pending_messages_request - END - DURATION: {$executionTime} - RESULT: no_pending_messages{$logContext} - REASON: {$lookupResult['reason']} - GLOBAL_PROCESSING_COUNT: {$lookupResult['global_processing_count']} - PROVIDER_PROCESSING_COUNT: {$lookupResult['provider_processing_count']} - RECENT_PROCESSING_COUNT: {$lookupResult['recent_processing_count']} - STALE_PROCESSING_COUNT: {$lookupResult['stale_processing_count']} - OLDEST_PROCESSING_AGE_SECONDS: {$this->formatNullableLogValue($lookupResult['oldest_processing_age_seconds'])} - OLDEST_PROCESSING_SMS_ID: {$this->formatNullableLogValue($lookupResult['oldest_processing_sms_id'])} - ELIGIBLE_IN_CURRENT_WINDOW: {$lookupResult['eligible_in_current_window']} - EXPIRED_IN_CURRENT_WINDOW: {$lookupResult['expired_in_current_window']} - UNASSIGNED_IN_CURRENT_WINDOW: {$lookupResult['unassigned_in_current_window']} - EXPIRED_COUNT: {$lookupResult['expired_count']} - ASSIGNED_COUNT: {$lookupResult['assigned_count']} - COMPLETED_COUNT: {$lookupResult['completed_count']} - FUTURE_COUNT: {$lookupResult['future_count']} - OLDEST_EXPIRED_AGE_SECONDS: {$this->formatNullableLogValue($lookupResult['oldest_expired_age_seconds'])} - OLDEST_PENDING_AGE_SECONDS: {$this->formatNullableLogValue($lookupResult['oldest_pending_age_seconds'])} - PENDING_WINDOW_SECONDS: {$lookupResult['pending_window_seconds']} - PROCESSING_LOCK_SECONDS: {$lookupResult['processing_lock_seconds']}"
            );
